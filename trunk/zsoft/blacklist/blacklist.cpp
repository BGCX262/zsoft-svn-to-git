...
int main(int argc, char* argv[])
{
	...
	system("touch /ezxlocal/download/mystuff/.blockedcalls");
					...
					if(pcs->connecttype == gsm)
					{
						switch(pcs->connectstatus)
						{
							case incomingcall:
								...
								//system(QString("echo \"%1\" >> /mmc/mmca1/llamadas; date +'%d-%m-%Y %H:%M:%S' >> /mmc/mmca1/llamadas").arg(pcs->callno));
								...
								if ( ltype == "Blacklist" )
								{
									...
									QString blockunknown = blacklist.readEntry ( QString("zPhoneFirewall"), QString("BlockUnknown"), "" );
									if ( blockunknown == "Yes" )
									{
										if ( QString("%1").arg(pcs->callno) == "" )
										{
											TAPI_VOICE_RejectCall( pcs->numofcall, 0 );
											//TAPI_VOICE_DropAllCall();
											system(QString("echo \"unknown\" >> /ezxlocal/download/mystuff/.blockedcalls;"
												"date +'%d-%m-%Y %H:%M:%S' >> /ezxlocal/download/mystuff/.blockedcalls"));
										}
									}
									if ( answer == "NO" )
									{
										TAPI_VOICE_RejectCall( pcs->numofcall, 0 );
										//TAPI_VOICE_DropAllCall();
										system(QString("echo \"%1\" >> /ezxlocal/download/mystuff/.blockedcalls;"
											"date +'%d-%m-%Y %H:%M:%S' >> /ezxlocal/download/mystuff/.blockedcalls").arg(pcs->callno));
									}
								}
